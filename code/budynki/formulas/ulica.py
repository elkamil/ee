...
def ulica(line):
    line = re.sub(r'\sG[Aa]\s', ' 6a ', re.sub(r'\sG[gG]\s', ' 6g ', re.sub(r'\sG[dD]\s',
                  ' 6d ', re.sub(r'G[bB]', '6b', re.sub(r'%', '9b', line.splitlines()[0])))))
    ...
    if j_ulica[0] and nr_domu_code:
        ulica_code = re.sub(r'^ul.\s?', '', re.sub(r'\s+', '', re.sub(r'[Źź]', 'z',
                            re.sub(r'[Żż]', 'z', re.sub(r'[Śś]', 's', re.sub(r'[óÓ]', 'o', re.sub(r'[Ńń]',
                                   '', re.sub(r'[Łł]', 'l', re.sub(r'[Ćć]', 'c', re.sub(r'[Ęę]', 'e',
                                              re.sub(r'[ąĄ]', 'a', j_ulica[0]))))))))))).lower()
