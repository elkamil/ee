import re
from mp.variables import x_nr_kw
...
def kw(line):
    if AF.search(line):
        ...
        if (len(re.findall('(,|;)', res12prim2)) == 0):
            x_nr_kw.append(re.sub(r'\s', '', re.sub(r'l', '/', res12prim2)))
            d_ksiega_pierwsza = ""
            ...
            x_nr_kw.append(re.sub(r'\s', '', d_ksiega_trzecia))
        else:
            x_nr_kw.append('')
            d_ksiega_pierwsza = ""
    else:
        x_nr_kw.append('')
        d_ksiega_pierwsza = ""
    return x_nr_kw, d_ksiega_pierwsza
