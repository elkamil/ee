import re
from lokale_mieszkalne.variables import af_nr_kw
...
def kw(line):
    if AF.search(line):
        ...
        if (len(re.findall('(,|;)', res12prim2)) == 0):
            af_nr_kw.append(re.sub(r'\s', '', re.sub(r'l', '/', res12prim2)))
            d_ksiega_pierwsza = ""
            ...
            af_nr_kw.append(re.sub(r'\s', '', d_ksiega_trzecia))
        else:
            af_nr_kw.append('')
            d_ksiega_pierwsza = ""
    else:
        af_nr_kw.append('')
        d_ksiega_pierwsza = ""
    return af_nr_kw, d_ksiega_pierwsza
